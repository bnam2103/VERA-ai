...
/* =========================
   SESSION + REQUEST IDS — see utils/ids.js
   ----------------------------------------------------------------------
   Extracted during the stabilization-stage modularization pass
   (2026-05-27). The following identifiers are now defined in
   `utils/ids.js`, which `index.html` loads BEFORE this file:

       VERA_SESSION_STORAGE_KEY, BMO_SESSION_STORAGE_KEY,
       getSessionScopedId, setSessionScopedId, getSessionId,
       _VERA_SESSION_STORAGE_KEYS_ALL, resetVeraAndBmoSessionIdsForTab,
       VERA_LAST_REQUEST_IDS, newVeraRequestId, recordVeraRequestId

   utils/ids.js also runs:
     - ?session=new URL handler
     - PART 2 `[VERA][SESSION]` verification log

   Since both files are loaded as classic scripts they share the global
   lexical environment, so bare-identifier references below
   (`getSessionId()`, `VERA_SESSION_STORAGE_KEY`, …) keep resolving
   exactly the way they did before the split. Window aliases are also
   set inside utils/ids.js as belt-and-braces for `typeof window.X` callers.
========================= */

/* PART 11: window.veraConcurrencyDebug() — moved to debug/voiceDebug.js
   (Stage 4, 2026-05-27). The DevTools probe now lives next to
   dumpVeraVoiceState / resetVeraVoiceRuntimeState; both still resolve
   `getSessionScopedId`, `VERA_LAST_REQUEST_IDS`, `workModeReasoningAbortControllers`,
...
/* logInputLimitDebug → moved to utils/logging.js (Stage 3, 2026-05-27). */
...
/* logVeraCapabilityFailure + logCapabilityFallbackDebug → moved to
 * utils/logging.js (Stage 3, 2026-05-27). */
...
/* looksLikeNewsSearchRequest, pendingNewsStatusBubble,
 * pendingNewsStatusTimerId, pendingNewsStatusToken,
 * PENDING_NEWS_STATUS_TIMEOUT_MS, PENDING_NEWS_STATUS_TEXT,
 * _clearPendingNewsStatusTimer, armPendingNewsStatusBubble,
...
/* interruptBargeInLatched → moved to voice/interruption.js (Stage 6, 2026-05-27). */
...
/* Fast VAD barge-in state (vadFastStopArmed, vadFastStopFiredAt,
 * vadFastStopTtsStoppedAt, vadFastStopAsrFinalAt, vadFastStopTtsId)
 * → moved to voice/interruption.js (Stage 6, 2026-05-27). */

/* logBargeInLatencyDebug → moved to utils/logging.js (Stage 3, 2026-05-27). */

/* =========================================================================
 *  INTERRUPTION / BARGE-IN DEBUG + TIMING + CANCEL-SOURCE
 *  → moved to voice/interruption.js (Stage 6, 2026-05-27).
 *
 *  Moved bindings:
 *    const _veraInterruptDebugLastAt,
 *    let _veraTtsCancelSource, let _veraInterruptDelayTrace,
 *    let _veraInterruptAttemptSeq,
 *    let interruptBargeInLatched (moved from its earlier siting),
 *    let vadFastStopArmed / vadFastStopFiredAt / vadFastStopTtsStoppedAt
 *      / vadFastStopAsrFinalAt / vadFastStopTtsId,
 *    function isVeraInterruptDebugEnabled,
 *    function logVeraInterruptDebug,
 *    function _newInterruptAttemptId,
 *    function _resetInterruptDelayTrace,
 *    function _ensureInterruptDelayTrace,
 *    function _recordInterruptTimingPoint,
 *    function _flushInterruptDelayTrace,
 *    function _logVoiceStateTransition,
 *    function _logTtsCancelSourceTrace,
 *    function resetVadFastStopState,
 *    function fastStopTtsOnVadOnly,
 *    function interruptTranscriptNewTtsId,
 *    function interruptSpeech.
 *
 *  RAF gap tracker, news-panel render flag, and per-NDJSON debug context
 *  remain here because they are written by hot-path code outside the
 *  interruption layer (detectInterrupt RAF loop and the news panel
 *  renderer). The moved trace helpers still read them at call time
...
/* News-panel render timing flag `_veraNewsPanelRenderInFlight` moved to
 * news/newsPanel.js (Stage 10, 2026-05-27). Still read here as a bare
 * identifier through the shared classic-script global lexical env
 * (`duringNewsRender` in interrupt-debug log payloads). */
...
/* ---------------------------------------------------------------------
 * Tab-local voice-runtime diagnostics — see debug/voiceDebug.js
 *
 * The two DevTools commands `window.dumpVeraVoiceState()` and
 * `window.resetVeraVoiceRuntimeState()` were extracted during the
 * stabilization-stage modularization pass (2026-05-27, Stage 4).
 * They still attach themselves to `window` and still read/mutate the
 * same app.js `let`-declared bindings (`mainTtsPlaybackActive`,
 * `interruptBargeInLatched`, `_veraTtsCancelSource`, …) through the
 * shared classic-script global lexical environment at CALL time.
 * debug/voiceDebug.js is loaded AFTER app.js in index.html.
 * ------------------------------------------------------------------- */
...
/* logVoiceFirstAudio + logVoiceMainReplyAudio + logVoicePipe → moved to
 * utils/logging.js (Stage 3, 2026-05-27). They still reach `voiceUxTurn`
...
function voiceTranscriptDebugEnabled() {
  return safeGetLocalStorage("VERA_DEBUG_TRANSCRIPTS") !== "0";
}
...
function voicePartialAsrDoneLogEnabled() {
  return safeGetLocalStorage("VERA_DEBUG_PARTIAL_ASR_DONE") !== "0";
}
...
function browserAsrStuckDebugEnabled() {
  return safeGetLocalStorage("VERA_DEBUG_BROWSER_ASR_STUCK") === "1";
}
...
/* logVoiceTranscript + logFinalTranscriptSentToLlm → moved to
 * utils/logging.js (Stage 3, 2026-05-27). `voiceTranscriptDebugEnabled`
 * remains in app.js (read-only gate, no extraction value). */

/* =========================================================================
 *  TURN TEXT INTEGRITY  (Stabilization Stage 1, 2026-05-27)
 *
 *  Moved to utils/logging.js (Stage 3, 2026-05-27):
 *    - _turnTextIntegrityEnabled
 *    - _veraTurnSeq + _nextVeraTurnId
 *    - logTurnTextIntegrity (+ window alias)
 *
 *  `_readLatestUserBubbleText` (below) STAYS in app.js because four other
 *  Work-Mode bubble-preservation sites also call it; the moved logger
 *  reaches it via the shared classic-script global lexical environment.
 *
...
/* logTurnTextIntegrity → moved to utils/logging.js (Stage 3, 2026-05-27).
 * window.logTurnTextIntegrity alias is now set inside utils/logging.js. */
...
function _workModeCommandDisplayTextEnabled() {
  ...
  return safeGetLocalStorage("VERA_DEBUG_WM_DISPLAY") !== "0";
}
...
/* browserAsrSupported + getSpeechRecognitionLang → moved to voice/asr.js
 * (Stage 7, 2026-05-27). */
...
/* VERA_SETTING_ASR_SILENCE_MS_KEY, VERA_SETTING_ASR_MODE_KEY,
 * VERA_SETTING_MAIN_ASR_PARTIAL_MIN_CHARS_KEY → moved to voice/asr.js
 * (Stage 7, 2026-05-27). */
...
/* isLikelyGoogleChrome + isNarrowViewport + browserAsrPreferred → moved
 * to voice/asr.js (Stage 7, 2026-05-27).
 *
 * `browserAsrPermanentlyDisabled` (the let) stays here because it is
 * mutated by Web Speech `not-allowed` / `service-not-allowed` error
 * handlers throughout the SR lifecycle code. */

/* getVeraAsrSilenceMs, setVeraAsrSilenceMs, _normalizeVeraAsrMode,
 * getVeraAsrMode, setVeraAsrMode, VERA_ASR_MODE_DEFAULT,
 * VERA_ASR_MODE_VALID, HYBRID_POLICY, isHybridAsrMode,
 * isWhisperAsrMode, isStreamingAsrMode → moved to voice/asr.js
 * (Stage 7, 2026-05-27). */

/* ASR transcript classifier / selector / normalizer:
 *   _splitCancelPrefix, decideAsrFinalizationMode,
 *   _normalizeForCompare, _levenshtein, _tokenOverlapRatio,
 *   _looksHallucinated, chooseBestTranscript,
 *   normalizeCommandTranscript, plus the supporting regex/array
 *   constants (ASR_RISKY_VOCAB_RE, ASR_ORDINAL_OR_RANGE_RE,
 *   ASR_EXPLICIT_ACCURATE_RE, ASR_STATE_CHANGING_RE,
 *   ASR_CANCEL_ONLY_RE, ASR_CANCEL_PREFIX_RE,
 *   ASR_COMMAND_NORMALIZATIONS) and the matching window.* aliases
 *   (window.decideAsrFinalizationMode, window.chooseBestTranscript,
 *   window.normalizeCommandTranscript, window.getVeraAsrMode,
 ...
 * `mainAsrPartialMinChars` (the let) stays here because the browser SR
 * lifecycle code reads it on every partial event. */

function isWorkModeMuteEnabled() {
  return safeGetLocalStorage(VERA_SETTING_WORKMODE_MUTE_KEY) === "1";
}

function setWorkModeMuteEnabled(on) {
  safeSetLocalStorage(VERA_SETTING_WORKMODE_MUTE_KEY, on ? "1" : "0");
...
function setTextGuideRotatorEnabled(on) {
  safeSetLocalStorage(VERA_SETTING_TEXT_GUIDE_ROTATOR_KEY, on ? "1" : "0");
...
function setPlanningDeadlineTimerEnabled(on) {
  safeSetLocalStorage(VERA_SETTING_PLANNING_DEADLINE_TIMER_KEY, on ? "1" : "0");
...
function replaceStableLaneIdForPanelIndex(idx) {
  initWorkModeStableLaneIdSlots();
  const i = Number(idx);
  const next = allocateWorkModeStableLaneId();
  if (!Number.isFinite(i) || i < 0) return next;
  while (workModeStableLaneIdByIdx.length <= i) {
    workModeStableLaneIdByIdx.push("");
  }
  workModeStableLaneIdByIdx[i] = next;
  return next;
}
...
/* WORK_CHECKLIST_STORAGE_KEY, WORK_CHECKLIST_COMPLETED_COLLAPSED_KEY,
 * workChecklistSyncTimer, workChecklistHydrationPromise,
 * workChecklistLocalMutationVersion, workChecklistSyncInFlight,
 * markWorkChecklistLocalMutation, readChecklistItemsFromStorage,
 * queueWorkChecklistSyncToServer, syncWorkChecklistToServerNow,
 * flushWorkChecklistSyncBeforeCommand, hydrateWorkChecklistFromServer
...
/* =========================================================================
   ...
   ─────────────────────────────────────────────────────────────────────
   The close-orchestration block (MIN_REASONING_PANELS,
   REASONING_RECENTLY_CLOSED_STACK_MAX, recentlyClosedReasoningPanels,
   logReasoningCloseDebug, logReasoningClosePolishDebug) has been moved
   to workmode/panels.js (Stage 8, 2026-05-27). The voice-turn lifecycle
   helpers below (finalize/finish/asr-mode/mic-state/active-user-bubble
   + reasoningCloseVoiceLifecycleSeq + logReasoningCloseVoiceLifecycle)
   remain here because they are tightly coupled to the ASR finalize path
   and continuous-listening restart.
   ========================================================================= */

/* logReasoningCloseDebug, logReasoningClosePolishDebug,
 * REASONING_CLOSE_TURN_LOCK_MS, _lastReasoningCloseLock → moved to
 * workmode/panels.js (Stage 8, 2026-05-27). */
 ...
 * _setReasoningCloseLock, _peekReasoningCloseLock,
 * _isGenericBlankReasoningPanelLabel, _isBlankReasoningPanelElement
 * → moved to workmode/panels.js (Stage 8, 2026-05-27). */

/* snapshotReasoningLaneRegistryForDebug, invalidateClosedReasoningLaneIdentity,
 * readPersistedReasoningPanelTitlesForDebug, _normalizeBlankPanelNamesInOrder,
 * _pickActivePanelInfoAfterRefill, getReasoningPanelOrder,
 * getReasoningTabTopicLabelSafe, snapshotReasoningPanelForUndo,
 * pickReplacementActivePanelInfo, refillReasoningPanelsToMinimum,
 * closeReasoningPanelsByVisualIndices, closeReasoningTab
 * → moved to workmode/panels.js (Stage 8, 2026-05-27). */

/* (body of invalidateClosedReasoningLaneIdentity moved to workmode/panels.js) */

/* readPersistedReasoningPanelTitlesForDebug moved to workmode/panels.js */
/* (body of _normalizeBlankPanelNamesInOrder moved to workmode/panels.js) */
...
/* (bodies of _pickActivePanelInfoAfterRefill, getReasoningPanelOrder,
 * getReasoningTabTopicLabelSafe, snapshotReasoningPanelForUndo,
 * pickReplacementActivePanelInfo, refillReasoningPanelsToMinimum
 * moved to workmode/panels.js) */
...
/* =========================
   ...
   CHECKLIST_ORDINAL_WORD_MAP + CHECKLIST_* regex constants,
   _checklistWordOrDigitOrdinal, parseChecklistOrdinals,
   _checklistDomState, logChecklistIntentDebug,
   detectChecklistActionIntent, isLikelyWorkChecklistEditIntent
   moved to workmode/checklist.js (Stage 9, 2026-05-27).
========================= */
...
/* createWorkChecklistDragHandle, WORK_CHECKLIST_SUBITEM_INDENT_THRESHOLD_PX,
 * workChecklistDragSession, readChecklistItemsFromStorageSafe,
 * writeChecklistItemsToStorageSafe, isChecklistDescendant,
 * applyChecklistNestingFromDrag, workChecklistInsertBeforeFromY,
 * persistWorkChecklistOrderFromDom,
 * applyWorkChecklistCompletedCollapseFromStorage,
 * wireWorkChecklistCompletedCollapse, ensureWorkChecklistListDnD,
 * normalizeWorkChecklistLeadingPlaceholderInStorage,
 * pruneInteriorEmptyOngoingItems, ensureWorkChecklistTrailingEmptyOngoing,
 * insertWorkChecklistEmptyOngoingAfter, loadWorkChecklistItems,
 * persistWorkChecklistToggle, persistWorkChecklistUpdateText,
 * persistWorkChecklistRemove moved to workmode/checklist.js
 * (Stage 9, 2026-05-27). */


/* WORK_CHECKLIST_HELP_PLAN_MAX_ITEMS, WORK_CHECKLIST_SYNC_PREVIEW_MAX_CHARS,
 * workChecklistSyncPreviewEditing, workChecklistSyncPlanVersion,
 * workChecklistSyncConsumedPlanVersion, workChecklistSyncPendingMarkdown,
 * workChecklistSyncPendingPlanMeta, workChecklistSyncCommandSeq,
 * activeWorkChecklistSyncCommand, lastCompletedWorkChecklistSyncCommandTurn
 * moved to workmode/checklist.js (Stage 9, 2026-05-27). */

/* NON_CANCELABLE_AFTER_COMMIT_ACTIONS, lastCommittedNonCancelableAction,
 * logChecklistActionCommitDebug, commitNonCancelableAction,
 * wasNonCancelableActionRecentlyCommitted moved to workmode/checklist.js
 * (Stage 9, 2026-05-27). */

/* planSyncPreviewRows, getPlanSyncPanelMetaForLane, logPlanSyncDebug,
 * logSyncVoiceTurnDebug, veraDebugSyncStateSnapshot (+ the
 * window.__veraDebugSyncState aliasing block) and
 * describePlanSyncActiveContext moved to workmode/checklist.js
 * (Stage 9, 2026-05-27). */

/* collectWorkChecklistOngoingTexts, workChecklistHasAnyStoredItems,
 * syncWorkChecklistEraseButton, syncWorkChecklistHelpPlanButton
 * moved to workmode/checklist.js (Stage 9, 2026-05-27). */

/* planSyncPanelGenerationInfo, getActivePlanSyncBlockingState,
 * __syncPlanButtonRefreshTimer, scheduleSyncPlanButtonRefresh,
 * syncWorkChecklistSyncPlanButton, getLatestWorkModeReasoningMarkdown,
 * getLatestMarkdownInReasoningScroll, isChecklistSyncHeadingText,
 * listItemsToChecklistMarkdown, renderedChecklistMarkdownFromPanel,
 * getWorkModeReasoningMarkdownCandidates,
 * getWorkChecklistSyncSourceCandidate,
 * getWorkChecklistSyncSourceMarkdown, normalizeChecklistLineText
 * moved to workmode/checklist.js (Stage 9, 2026-05-27). */

/* buildChecklistProposalFromMarkdown, formatChecklistProposalText,
 * parseChecklistProposalText moved to workmode/checklist.js
 * (Stage 9, 2026-05-27). */

/* setWorkChecklistSyncPreviewEditing, showWorkChecklistSyncPreview,
 * hideWorkChecklistSyncPreview, applyWorkChecklistSyncPreview,
 * eraseEntireWorkChecklist moved to workmode/checklist.js
 * (Stage 9, 2026-05-27). */

/* runWorkChecklistSyncFromLatestPlan, workChecklistPlanHintTimer,
 * flashWorkChecklistPlanHint, buildWorkChecklistHelpPlanUserMessage,
 * workChecklistPlanRequestInFlight, isWorkChecklistPlanShortcutIntent,
 * isWorkChecklistSyncCommandIntent moved to workmode/checklist.js
 * (Stage 9, 2026-05-27). */
...
/* queueWorkChecklistRowEnterAnimation moved to workmode/checklist.js
 * (Stage 9, 2026-05-27). */
...
function shouldBlockOrdinalProblemLaneActivation(userText) {
  ...
  if (
    /\b(?:go\s+back\s+to|go\s+to|jump\s+to|switch\s+to|change\s+to|open|activate|show|select|use)\b[^.?!]{0,96}\b(?:reasoning\s+)?(?:panel|tab|lane|page)\b/i.test(
      low
    )
  ) {
    return false;
...
function applyActionPayload(data) {
  ...
  /* News panel UI command — pure open/close, never a Work Mode reasoning
     panel switch. Backend emits this when the user says "open the news
     panel", "show news tab", "close news". The render path reuses the
     existing News Results panel surface (or hides it). Empty headlines
     are fine — the user asked for a UI surface, not a briefing. */
  if (payload?.panel_type === "news_panel_ui") {
    const op = (payload.op || "open").toLowerCase();
    if (op === "close") {
      hideSidePanel();
    ...
    /* op === "open" — render the news panel shell. If we have no fresh
       results, show an empty headlines placeholder so the panel surface
       exists and the user can use the tab controls. We deliberately do
       NOT trigger a Serper call here — the user only asked for the UI. */
    const openPayload = {
      panel_type: "news_results",
      title: payload.title || "News",
      query: payload.query || "Latest headlines",
      default_tab: "news",
      news_results: [],
      results: [],
      images: [],
  ...
  if (payload?.panel_type === "work_mode_reasoning") {
    ...
    /* PART 3: backend can also drive panel close (e.g. the LLM router sent
       back work_mode.reasoning_close_panel because the heuristic didn't
       fire, or via /infer for a typed command). The payload carries the
       parsed command shape from PART 14 and we re-run the executor here
       so the source of truth for index resolution + refill + active
...
function cancelBrowserInterruptTtsOnly() {
  ...
  _logVoiceStateTransition(
    ...
    "interrupted",
    "interrupt_browser_or_single_asr",
  ...
  _logVoiceStateTransition(
    "interrupted",
    "listening",
    "cancelBrowserInterruptTtsOnly_complete",
    "cancelBrowserInterruptTtsOnly"
  );
  listening = true;
  processing = false;
  ...
  /* PART 1 — flush trace on next tick to capture any tail t10/t11 events. */
  try {
    setTimeout(
      () => _flushInterruptDelayTrace("cancelBrowserInterruptTtsOnly_complete"),
      0
    );
  } catch (_) {}
}
...
function onBrowserInterruptBargeInFromDetect(event) {
  ...
  /* PART 1 — t5: barge-in handler entered. */
  _recordInterruptTimingPoint("t5_interruptSpeech_entered", {
    extra: { gatePath: "onBrowserInterruptBargeInFromDetect" },
  });
  ...
  /* PART 1 — t3: first final fragment from the browser SR (if any).
     Useful to verify TTS cancel did NOT wait for this point. */
  if (_sawFinal) {
    _recordInterruptTimingPoint("t3_final_transcript_available", {
      extra: { finalLen: finalP.length },
    });
  }
...
/* interruptSpeech → moved to voice/interruption.js (Stage 6, 2026-05-27). */
    ...
    if (now - audioStartedAt > 200) {
      ...
      if (
        ...
        if (browserAsrParallelMode) {
          ...
          /* PART 1 — t4: gate passed, intent confirmed (VAD parallel). */
          _recordInterruptTimingPoint("t4_interrupt_intent_detected", {
            autoStart: true,
            extra: { gate: "heuristic_fast_stop" },
          });
...
function startInterruptCapture() {
  ...
  /* PART 12: use shared opus-preferred recorder helper so interrupt-path
     audio matches the quality of the main + hybrid recorders. */
...
async function prearmInterruptCaptureForTts({ turnId = "", ttsId = "" } = {}) {
  ...
  try {
    ...
    interruptPrearmChunks = [];
    interruptPrearmStartedAt = performance.now();
    /* PART 12: opus + bitrate so pre-arm captures sound as good as the
       main recorder. The "opening syllables get clipped" risk is mitigated
       by the prearm-start latency timing, not the encoder. */
...
async function handleInterruptUtterance(blob) {
  ...
  processing = true;
  /* PART 3 — log the "interrupted/listening" → "processing/thinking" hop
     that happens once the interrupt recorder closes the utterance. */
  _logVoiceStateTransition("listening", "processing", "interrupt_utterance_upload", "handleInterruptUtterance");
  waveState = "idle";
    ...
    try {
      await playTtsFromApi(data, {
        onPlayStart: () => {
          ...
          /* PART 3 — interrupt-reply TTS starts: state hop into speaking. */
          _logVoiceStateTransition("processing", "speaking", "interrupt_reply_play_start", "playInterruptAnswer.onPlayStart");
          waveState = "speaking";
...
/* =========================================================================
 *  MAIN-TTS RUNTIME STATE + BOOKKEEPING + CANCELLATION
 *  → moved to voice/ttsQueue.js (Stage 5, 2026-05-27).
 *
 *  Moved bindings:
 *    let activeMainTtsBufferSources, let mainTtsPlaybackActive,
 *    let mainTtsPlaybackToken, let activeNdjsonBodyReader,
 *    function registerMainTtsBufferSource,
 *    function stopAllMainTtsWebAudio,
 *    function cancelMainTtsPlayback.
 *
 *  isAssistantTtsPlaying remains here because many non-TTS sites call
 *  it; it still reads the moved state via the shared classic-script
...
function interruptAssistantPipelineForTypedMessage() {
  /* PART 3/4 — typed keyboard barge-in is structurally similar to voice
     barge-in: log the state transition so the trace shows where TTS got
     cancelled when the user typed instead of spoke. */
  _logVoiceStateTransition(
    typeof waveState !== "undefined" ? waveState : null,
    "interrupted",
    "typed_pipeline_barge_in",
...
function cancelVoicePipelineAndResetState() {
  /* PART 3 — full pipeline reset (e.g. mode swap, error recovery). */
  _logVoiceStateTransition(
    typeof waveState !== "undefined" ? waveState : null,
    "idle",
    "voice_pipeline_reset",
...
function resumeAfterAssistantReplyPlayback() {
  /* PART 3 — natural end of an assistant reply: speaking → listening/idle. */
  _logVoiceStateTransition(
    typeof waveState !== "undefined" ? waveState : null,
    listeningMode === "ptt" ? "idle" : "listening",
    "assistant_reply_playback_end",
...
function resumeListeningAfterInterruptPlayback() {
  /* PART 3 — natural end of an INTERRUPT reply playback. */
  _logVoiceStateTransition(
    typeof waveState !== "undefined" ? waveState : null,
    listeningMode === "ptt" ? "idle" : "listening",
    "interrupt_reply_playback_end",
    "resumeListeningAfterInterruptPlayback"
  );
  browserAsrMainNetworkRetries = 0;
  processing = false;
...
/* playTtsUrlSequenceGapless → moved to voice/ttsQueue.js (Stage 5, 2026-05-27). */
...
/* PART 11 — final-result settle delay.
 *
 * The browser SpeechRecognition silence timer (browserAsrMainSilenceMs)
 * decides "user has paused long enough — finalize". But Web Speech can
 * still emit one or two more `isFinal` results in the 200-400ms after
 * silence is observed, especially for long dictation. In hybrid/whisper
 * modes we want to capture those tail finals before freezing the
 * transcript and (for hybrid) dispatching a Whisper verification request,
 * because the difference between "remove first and third" and "remove
 * first and fifth" might be carried by that tail. Streaming mode keeps
 * its existing latency profile (no extra wait).
 */
function _finalResultSettleMs() {
  const mode = getVeraAsrMode();
  if (mode === "hybrid") return 300;
  if (mode === "whisper") return 280;
  return 0;
}
...
function scheduleMainBrowserEndOfUtterance() {
  ...
  const settleMs = _finalResultSettleMs();
  const totalMs = browserAsrMainSilenceMs + settleMs;
...
async function finalizeMainBrowserTranscript(text) {
  ...
  /* PART 5+6+7 — hybrid finalization decision.
     ...
     to "whisper_verify". We resolve the hybrid sidecar blob (if any)
     before mutating UX state so the recorder finalizes deterministically. */
  const asrMode = getVeraAsrMode();
  const finalization = decideAsrFinalizationMode({
    browserTranscript: trimmed,
    browserConfidence: null,
    isInterruption: false,
    recentUiContext: { active_panel_id: null },
  });
  let hybridAudio = null;
  if (asrMode === "hybrid" && isHybridSidecarRunning()) {
    if (finalization.mode === "whisper_verify") {
      hybridAudio = await _stopAndCollectHybridSidecar();
    } else {
      _discardHybridSidecar();
    }
  }
  try {
    console.info("[asr_finalization_debug]", {
      stage: "main_browser_finalize",
      asr_mode: asrMode,
      hybrid_policy: HYBRID_POLICY,
      browser_final_transcript: trimmed.slice(0, 200),
      finalization_mode: finalization.mode,
      finalization_reason: finalization.reason,
      hybrid_audio_chunks: hybridAudio?.chunkCount || 0,
      hybrid_audio_duration_ms: hybridAudio?.durationMs ? Math.round(hybridAudio.durationMs) : 0,
    });
  } catch (_) {}

  if (finalization.mode === "cancel_only_immediate") {
    /* No LLM round-trip — caller already stopped TTS (the cancel happened
       during a non-interrupt main turn, which is rare but possible). */
    _discardHybridSidecar();
    stopAllBrowserSpeechRecognizers();
    processing = false;
    voiceUxTurn = null;
  ...
  if (finalization.mode === "whisper_verify" && hybridAudio?.blob) {
    /* PART 8: when the client decides this utterance warrants a Whisper
       pass, send the audio AND the browser transcript AND the
       request_whisper_verify=1 flag so the server runs Whisper, picks the
       best transcript via chooseBestTranscript(), and uses the selected
       transcript for routing. */
    formData.append("audio", hybridAudio.blob, "hybrid_utterance.webm");
    formData.append("request_whisper_verify", "1");
    formData.append("hybrid_browser_transcript", trimmed);
    try {
      console.info("[asr_mode_debug]", {
        stage: "hybrid_dispatch_with_whisper",
        bytes: hybridAudio.blob.size,
        mime_type: hybridAudio.mimeType,
      });
    } catch (_) {}
  }
  if (listeningMode === "ptt") {
...
function startInterruptBrowserPartialDetection() {
  ...
  interruptDetectRecognition.onresult = (event) => {
    ...
    if (combined !== lastCombined) {
      const _wasFirstInterim = !lastCombined;
      ...
      /* PART 1 — t1: first non-empty interim transcript starts the
         transcription leg of the timeline. t2 is refreshed on every
         subsequent interim so we can measure how long the user kept
         talking before TTS finally stopped. */
      if (_wasFirstInterim) {
        _recordInterruptTimingPoint("t1_interim_transcript_started", {
          autoStart: true,
          extra: { firstInterim: combined.slice(0, 64) },
        });
      }
      _recordInterruptTimingPoint("t2_interim_transcript_updated", {
        extra: { latestLen: combined.length },
      });
      ...
      if (wordGate || sustainGate) {
        /* PART 1 — t4: gate passed via browser ASR partials. */
        _recordInterruptTimingPoint("t4_interrupt_intent_detected", {
          autoStart: true,
          extra: {
  ...
  try {
    ...
    if (maybeHandleCloseReasoningPanelShortcut(routedText, { reason: "voice_interruption", isVoice: true })) {
      logInterruptTranscriptDebug("routed_to_normal_user_turn", {
        original_transcript: String(originalText || "").slice(0, 120),
        normalized_command_text: routedText.slice(0, 120),
        shortcut: "close_reasoning_panel",
        checklist_sync_intent_detected: false,
        sync_action_started: false,
        sync_source: "voice_interruption",
        active_panel_id_at_interruption: baseCtx.active_panel_id,
...
async function finalizeInterruptBrowserTranscript(text) {
  ...
  /* PART 14 — hybrid barge-in: when the user opted into Hybrid and the
     ...
     transcript. The cancel-only case was already short-circuited above
     (no /infer round-trip), so reaching here means the interruption is
     state-changing. */
  const _interruptAsrMode = getVeraAsrMode();
  formData.append("asr_mode", _interruptAsrMode);
  const _interruptDecision = decideAsrFinalizationMode({
    browserTranscript: routedText,
    isInterruption: true,
    browserConfidence: null,
  });
  formData.append("asr_finalization_mode", _interruptDecision.mode);
  formData.append("asr_finalization_reason", String(_interruptDecision.reason || ""));
  if (
    _interruptAsrMode === "hybrid"
    && _interruptDecision.mode === "whisper_verify"
    && Array.isArray(interruptChunks)
    && interruptChunks.length > 0
  ) {
    try {
      const _ttype = interruptChunks[0]?.type || "audio/webm";
      const _ibloop = new Blob(interruptChunks, { type: _ttype });
      if (_ibloop && _ibloop.size > 0) {
        formData.append("audio", _ibloop, "hybrid_interrupt.webm");
        formData.append("request_whisper_verify", "1");
        formData.append("hybrid_browser_transcript", routedText);
        try {
          console.info("[asr_mode_debug]", {
            stage: "hybrid_interrupt_dispatch_with_whisper",
            bytes: _ibloop.size,
            mime_type: _ttype,
          });
        } catch (_) {}
...
function startListening() {
  ...
  if (listeningMode === "continuous" && browserAsrPreferred()) {
    ...
    updateMuteInputButton();
    /* PART 5+7: hybrid mode runs MediaRecorder alongside browser ASR so a
       Whisper verification pass on the same audio is available later for
       risky/state-changing commands — without paying single-Whisper
       latency on every utterance. Streaming mode skips this entirely. */
    startHybridSidecarRecorderIfNeeded(micStream);
  ...
  /* PART 12 helper. */
  mediaRecorder = createVeraMediaRecorder(micStream);
...
async function beginPttRecordingNow() {
  ...
  /* PART 12 helper for PTT/non-browser-ASR path. */
  mediaRecorder = createVeraMediaRecorder(micStream);
  mediaRecorder.ondataavailable = (e) => audioChunks.push(e.data);
  mediaRecorder.onstop = handleUtterance;
  mediaRecorder.start();
...
function wireVeraSettingsPanel() {
  ...
  const asrHybridBtn = document.getElementById("vera-setting-asr-hybrid");
  ...
  const applyAsrModeUi = (mode) => {
    const normalized = _normalizeVeraAsrMode(mode);
    const setRadio = (btn, on) => {
      if (!(btn instanceof HTMLButtonElement)) return;
      btn.classList.toggle("is-active", on);
      btn.setAttribute("aria-pressed", on ? "true" : "false");
    };
    setRadio(asrStreamingBtn, normalized === "streaming");
    setRadio(asrWhisperBtn, normalized === "whisper");
    setRadio(asrHybridBtn, normalized === "hybrid");
  };
  ...
  const wireAsrModeRadio = (btn, mode) => {
    if (!(btn instanceof HTMLButtonElement)) return;
