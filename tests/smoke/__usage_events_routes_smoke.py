"""Smoke: POST /api/usage/events (behavioral analytics MVP).
...
from auth.usage_events_db import MAX_PROP_STRING_LEN, create_usage_event, sanitize_event_props
...
with patch("auth.usage_events_routes.create_usage_event") as mock_create:
    ...
    res_anon = client.post(
        ...
        json={
            ...
            "event_props": {"page": "vera", "authenticated": False},
        },
    )
...
res_bad_type = client.post(
    "/api/usage/events",
    json={"session_id": "sess-x", "event_type": "action_success"},
)
