...
const _PANEL_NAV_ORDINAL_TO_VISUAL = {
  first: 1, second: 2, third: 3, fourth: 4, fifth: 5, sixth: 6, seventh: 7, eighth: 8,
  one: 1, two: 2, three: 3, four: 4, five: 5, six: 6, seven: 7, eight: 8,
  last: -1, previous: -2, prior: -2, next: -3,
};

function parsePanelNavigationCommand(text, panelCount) {
  const out = {
    intent: null,
    targetVisualIndex: null,
    targetTitleQuery: null,
    direction: null,
    rawCommandText: String(text || "").trim(),
  };
  const s = out.rawCommandText;
  if (!s) return out;
  const order = (() => {
    try {
      return getReasoningPanelOrder();
    } catch (_) {
      return [];
    }
  })();
  const N = Math.max(Number(panelCount) || 0, order.length);
  const activeVisual = order.find((p) => p.isActive)?.visualIndex || 1;

  if (typeof isPanelOpenNewReasoningPanelCommand === "function" && isPanelOpenNewReasoningPanelCommand(s)) {
    out.intent = "panel_open_new";
    return out;
  }

  const navOnly =
    typeof isNavigationOnlyPanelCommand === "function" ? isNavigationOnlyPanelCommand(s) : false;
  if (!navOnly) return out;

  const low = s.toLowerCase();
  if (/^\s*next\s+(?:reasoning\s+)?(?:panel|tab|space|page|lane)\s*[?.!]*\s*$/i.test(s)) {
    out.intent = "panel_navigate";
    out.direction = "next";
    out.targetVisualIndex = Math.min(activeVisual + 1, N || activeVisual + 1);
    return out;
  }
  if (/^\s*(?:previous|prior)\s+(?:reasoning\s+)?(?:panel|tab|space|page|lane)\s*[?.!]*\s*$/i.test(s)) {
    out.intent = "panel_navigate";
    out.direction = "previous";
    out.targetVisualIndex = Math.max(activeVisual - 1, 1);
    return out;
  }
  if (
    /\b(?:go\s+back\s+to|go\s+to|jump\s+to|switch\s+to|change\s+to|move\s+to)\s+(?:the\s+)?(?:previous|prior|last)\s+(?:reasoning\s+)?(?:panel|tab|space|page|lane)\b/i.test(
      s
    )
  ) {
    out.intent = "panel_navigate";
  ...
  let m = low.match(/\b(?:panel|tab|space|page|lane)\s*#?\s*(\d+)\b/);
  if (m) {
    out.intent = "panel_navigate";
    out.targetVisualIndex = parseInt(m[1], 10);
    return out;
  }

  m = low.match(
    /\b(first|second|third|fourth|fifth|sixth|seventh|eighth|one|two|three|four|five|six|seven|eight)\s+(?:reasoning\s+)?(?:panel|tab|space|page|lane)\b/
  );
  if (m) {
    out.intent = "panel_navigate";
    out.targetVisualIndex = _PANEL_NAV_ORDINAL_TO_VISUAL[m[1]] || null;
    return out;
  }

  const titleFromHeuristic =
    typeof goToReasoningPanelQueryHeuristicUi === "function" ? goToReasoningPanelQueryHeuristicUi(s) : null;
  if (titleFromHeuristic) {
    out.intent = "panel_navigate";
    out.targetTitleQuery = titleFromHeuristic;
    return out;
  }

  m = s.match(
    /\b(?:go\s+back\s+to|go\s+to|jump\s+to|switch\s+to|change\s+to|move\s+to|show|select|use|open)\s+(?:the\s+|a\s+|my\s+)?(.+?)\s+(?:reasoning\s+)?panel\b/i
  );
  if (m) {
    const q = String(m[1] || "").trim();
    if (q && !/^(?:first|second|third|fourth|fifth|sixth|seventh|eighth|one|two|three|four|five|six|seven|eight)$/i.test(q)) {
      out.intent = "panel_navigate";
      out.targetTitleQuery = q.replace(/^(?:the\s+)/i, "").trim();
      return out;
    }
  }

  if (typeof resolvePanelNavigationVisualIndexFromText === "function") {
    const idx = resolvePanelNavigationVisualIndexFromText(s);
    if (Number.isFinite(idx) && idx >= 1) {
      out.intent = "panel_navigate";
      out.targetVisualIndex = idx;
      return out;
    }
  }
...
function executePanelNavigationCommand(parsed, opts = {}) {
  if (!parsed || !parsed.intent) {
    return { ok: false, failureReason: "no_intent" };
  }
  if (parsed.intent === "panel_open_new") {
    const opened = addReasoningTab({ source: opts.reason || "panel_navigation_open_new" });
    if (!opened) return { ok: false, failureReason: "open_failed" };
    const label =
      getReasoningTabTopicLabelSafe(opened) ||
      String(opened.dataset.laneLabel || "") ||
      "a new reasoning panel";
    return { ok: true, confirmation: `Opened ${label}.`, label, intent: "panel_open_new" };
  }

  const order = getReasoningPanelOrder();
  if (!order.length) return { ok: false, failureReason: "no_panels" };

  let visualIndex = parsed.targetVisualIndex;
  if (parsed.targetTitleQuery) {
    const hits = findReasoningPanelIndicesByTitleQuery(parsed.targetTitleQuery);
    if (hits.length === 1) {
      visualIndex = hits[0];
      ...
      return {
        ok: false,
        failureReason: "ambiguous_title",
        titleQuery: parsed.targetTitleQuery,
        matchedTitles: hits.map((vi) => order[vi - 1]?.label).filter(Boolean),
      };
    } else {
      return { ok: false, failureReason: "no_title_match", titleQuery: parsed.targetTitleQuery };
    }
  }

  if (!Number.isFinite(visualIndex) || visualIndex < 1 || visualIndex > order.length) {
    return {
      ok: false,
  ...
  const entry = order[visualIndex - 1];
  activateReasoningTab(entry.tabIndex, {
    commandText: parsed.rawCommandText,
    resolvedFrom: "panel_navigation_shortcut",
    requestedIndex: visualIndex,
  });
  const label = entry.label || `Panel ${visualIndex}`;
  return {
    ok: true,
    confirmation: `Switching to ${label}.`,
    label,
    visualIndex,
    tabIndex: entry.tabIndex,
    intent: "panel_navigate",
  };
}
...
function maybeHandleWorkModePanelNavigationShortcut(text, opts = {}) {
  ...
  const parsed = parsePanelNavigationCommand(text, order.length);
  const navIntentDetected = parsed.intent === "panel_navigate" || parsed.intent === "panel_open_new";
  const gatePassed = (() => {
    try {
  ...
  const hasReasoningPanelDom = order.length > 0 || parsed.intent === "panel_open_new";

  if (navIntentDetected) {
    try {
      console.info("[panel_navigation_detected]", {
        ...
        target_visual_index: parsed.targetVisualIndex,
        target_title_query: parsed.targetTitleQuery || null,
        direction: parsed.direction || null,
      });
    } catch (_) {}
  }

  if (!navIntentDetected) return false;

  if (shouldDeferPanelNavigationShortcutForMultiAction(text)) {
    try {
      console.info("[panel_navigation_shortcut_deferred]", {
        raw_text: String(text || "").slice(0, 240),
        input_source: inputSource,
        source,
  ...
  if (!gatePassed && (!navIntentDetected || !hasReasoningPanelDom)) {
    return false;
  }

  const exec = executePanelNavigationCommand(parsed, { userText: text, reason: source });
  const reply = exec.ok
    ? exec.confirmation
    : exec.failureReason === "ambiguous_title"
      ? `I found more than one panel matching "${parsed.targetTitleQuery}". Which one should I switch to?`
      : exec.failureReason === "no_title_match"
        ? `I couldn't find a panel matching "${parsed.targetTitleQuery}".`
        : exec.failureReason === "out_of_range"
          ? "That panel number isn't available."
          : exec.failureReason === "no_panels"
            ? "I don't see any reasoning panels yet."
            : "I couldn't switch panels.";
  ...
  if (exec.ok) {
    try {
      console.info("[panel_navigation_dispatched]", {
        ...
        source,
        intent: parsed.intent,
        target_visual_index: exec.visualIndex ?? parsed.targetVisualIndex,
        label: exec.label || null,
        confirmation: reply,
      });
    } catch (_) {}
  }

  if (opts.isVoice && typeof addBubble === "function") {
    try {
      addBubble(String(text || "").trim(), "user", { path: "panel-navigation-user" });
    } catch (_) {}
  } else if (!opts.isVoice && typeof addBubble === "function") {
    try {
      addBubble(String(text || "").trim(), "user");
    } catch (_) {}
  }
...
try {
  ...
  window.parsePanelNavigationCommand = parsePanelNavigationCommand;
  window.executePanelNavigationCommand = executePanelNavigationCommand;
  window.maybeHandleWorkModePanelNavigationShortcut = maybeHandleWorkModePanelNavigationShortcut;
} catch (_) {}
/* =========================================================================
